...
class FilterScheduler(driver.Scheduler):
    ...
    def _schedule(self, context, request_spec, filter_properties,
        ...
        # summerdaway 
        LOG.error(_("hosts %(selected_hosts)s") % locals())
        host_list = []
        for host in selected_hosts:
          somet = "%s" % (host.obj.host)
          somet = somet[1:]
          LOG.error(_("host %(somet)s") % locals())
          host_list.append(string.atoi(somet))
        LOG.error(_("host list %(host_list)s")%locals())
        threads = []
        for host in host_list:
          LOG.error(_("killing on host %(host)s")%locals())
          kill_on(host)
          #th = threading.Thread(target=kill_on, args=(host))
          #threads.append(th)
          #th.start()
          #thread.start_new_thread(kill_on, (host))
        #for th in threads:
        #  LOG.warning(_("waiting for thread %(th)s")%locals())
        #  th.join(30)
